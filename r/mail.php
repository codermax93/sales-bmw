...
switch( $_POST["form"] )
{
	default:
		...
		if ($_POST["files"] && trim($_POST["files"]) != "")
		{
			$l_images = explode("|+|+|",$_POST["files"]);
			foreach ($l_images as $l_image)
				$e_text .= "<a href='http://".$l_host."/".$l_path."/".$l_image."'>".$l_host."/".$l_path."/".$l_image."</a><br/>";
			
		}
		
		SendMail( "robot@".$l_host, $l_to," ".$_POST["title"], $e_text );
        ...
        @file_put_contents($_SERVER['DOCUMENT_ROOT']."/r/crm-".date("Y-m").".log", print_r($arSave, 1)."\r\n", FILE_APPEND);
